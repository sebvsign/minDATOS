...
def listar_trimestres(trimestres_list):
    contador = 0
    trimestres = []
    print("\n\t\tTrimestres Availables")
    for trimestre in trimestres_list:
        contador += 1
        trimestres.append(trimestre)
        print(str(contador) + ") " + str(trimestre))
    return trimestres
...
while True:          
    ...
    trimestresMenu = listar_trimestres(group_trimestre_column)
    print(estados)
    #df_trimestre_selected = df_estado_selected[df_estado_selected['Trimestre'] == trimestresMenu[int(opcion) - 1]]
    
    #df_temp = df_trimestre_selected[df_trimestre_selected['MaxTemp'].notna()]
    #df_max_temp = df_temp.groupby('Year')['MaxTemp'].max().reset_index(name='Max_Temp').astype(int)